(function (exports) {
  ...
  const { ContextTracker: ContextTracker$1, ExternalTokenizer: ExternalTokenizer$1, LRParser: LRParser$2 } = CM["@lezer/lr"];
  const { styleTags: styleTags$2, tags: tags$1 } = CM["@lezer/highlight"];
  CM["@lezer/common"];
  ...
  const elementContext = new ContextTracker$1({
    start: null,
  ...
  const tagStart = new ExternalTokenizer$1((input, stack) => {
  ...
  const commentContent = new ExternalTokenizer$1(input => {
  ...
  const endTag = new ExternalTokenizer$1((input, stack) => {
  ...
  function contentTokenizer(tag, textToken, endToken) {
    ...
    return new ExternalTokenizer$1(input => {
      // state means:
      // - 0 nothing matched
      // - 1 '<' matched
  ...
  const htmlHighlighting = styleTags$2({
  ...
  // This file was generated by lezer-generator. You probably shouldn't edit it.
  const parser$2 = LRParser$2.deserialize({
    version: 14,
  ...
  const { ContextTracker, ExternalTokenizer, LRParser: LRParser$1, LocalTokenGroup } = CM["@lezer/lr"];
  const { styleTags: styleTags$1, tags } = CM["@lezer/highlight"];
  ...
  const parser$1 = LRParser$1.deserialize({
    version: 14,
  ...
  const mixedHTMLParser = parser$2.configure({
    wrap: parseMixed(node => {
      return node.name == "ScriptText" ? {parser: parser$1} : null
    })
  });
  ...
  const {htmlLanguage} = CM["@codemirror/lang-html"];
